...
public class out extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FirebaseListOptions<expense> options = new FirebaseListOptions.Builder<expense>()
                ...
                .setLifecycleOwner(this) // 使用 this 而不是 out.this
                .setQuery(query, expense.class) // 改成 expense 而不是 expense.class
                .build();
        adapter = new FirebaseListAdapter<expense>(options) { // 明確指定泛型
            @Override
            protected void populateView(View v, expense model, int position)
            {
                ...
                name.setText("支出名稱：" + exp.getName());
                money.setText("支出金額：" + exp.getMoney());
                date.setText("日期：" + exp.getDate());
                time.setText("時間：" + exp.getTime());
                Picasso.get().load(exp.getImg_uri()).into(imageView);
            }
